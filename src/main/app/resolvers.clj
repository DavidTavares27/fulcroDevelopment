...
(def pokemons-list
  {:pokemons {:list/id :pokemons
              :list/pokemons (vec (range 1 21))}})

(defn normalize-pokemons [pokemons]
  (->> pokemons
       (map-indexed (fn [i pokemon] {(+ 1 i) {:pokemon/name (:name pokemon)
                                              :pokemon/url (:url pokemon)}}))
       (into (sorted-map))))

(defn pokemon-api-response->pokemons [response]
  (-> response
      :body
      (json/parse-string true)
      :results))

(pc/defresolver pokemon-resolver [_env {:list/keys [id]}]
  {::pc/input #{:list/id}
   ::pc/output [:list/id {:list/pokemons [:pokemon/name]}]}
  (let [response (client/get "https://pokeapi.co/api/v2/pokemon")
        pokemons (pokemon-api-response->pokemons response)]
    (normalize-pokemons pokemons)))
...
(def resolvers [ pokemon-resolver]) ;[person-resolver  list-resolver]
